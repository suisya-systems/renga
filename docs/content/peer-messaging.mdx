...
[`claude-peers-mcp`](https://github.com/happy-ryo/claude-peers-mcp) は `cwd` / `git_root` / `PID` の組み合わせからペアを推測しますが、renga-peers は **「ユーザーがこのタブに並べて置いた」という物理配置** そのものをスコープの拠り所にします。推測に頼らないので、同じディレクトリを複数案件で開いていても取り違えませんし、同じリポジトリを別の場所で再オープンしても重複しません。renga がペインのライフサイクルを把握している以上、古くなったペアを掃除する仕組みも追加で要りません。なお #289 以降、権威スコープは renga **セッション**です: `list_peers` は全タブを列挙し (自分のタブが先頭)、数値 pane id を宛先にすればメッセージはタブをまたいで届きます。タブが今も律しているのは**名前の解決**で、pane 名はタブ内でのみ一意なため、名前指定は他タブへ解決されません (「配送の仕様」参照)。
...
返ってきた `next_since` をそのまま次回の poll に使ってください。`events_dropped` が来たときだけ、1 回 `list_panes` で再同期してから続行します。ただし `list_panes` が返すのは呼び出し元自身のタブだけなので、他タブのペインで落ちたイベントはこの方法では埋め合わせられません。
