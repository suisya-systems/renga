...
[`claude-peers-mcp`](https://github.com/happy-ryo/claude-peers-mcp) infers peer scope from `cwd` / `git_root` / `PID` — heuristics that collide when the same repo is opened from two places. renga-peers uses **the renga tab as the authoritative scope**: the user literally put these panes next to each other, which is a stronger signal than "they share a cwd". renga owns pane lifecycle, so peer liveness is exact — no PID polling, no SQLite GC. Since #289 the authoritative scope is the renga **session**: `list_peers` enumerates every tab (your own first) and a numeric pane id reaches a peer in any tab. What the tab still governs is *name* resolution — pane names are unique per tab, so a bare name never resolves across tabs (see Delivery semantics).
...
Loop with the returned `next_since` cursor. If you ever receive an `events_dropped` event, re-sync once with `list_panes` before continuing — but that only refreshes the caller's own tab, so events dropped for panes in other tabs cannot be reconciled this way.
