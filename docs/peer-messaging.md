...
- **`poll_events` returns `events: []` before the timeout you expected** — A `types=[…]` filter only narrows what is returned; a non-matching event can still wake the long-poll and advance `next_since`. Re-issue the call with the returned cursor. If you receive `events_dropped`, re-sync once with `list_panes` — but note that `poll_events` is process-wide and delivers pane lifecycle from *every* tab, while `list_panes` returns only your own tab, so that re-sync cannot reconcile events dropped from another tab.
