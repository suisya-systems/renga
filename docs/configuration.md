...
| `anchor_row_offset` | u16 rows | `1` | `--ime-anchor-row-offset <ROWS>` | Rows the terminal caret is pushed below the focused pane's caret row so a **native** (non-overlay) IME candidate window clears the input line instead of butting into it. Applies only on conpty hosts (native Windows, or WSL under Windows Terminal); other terminals ignore it and keep the caret exactly on the resolved cell. The visible caret moves with the anchor — set `0` to pin it back onto the input cell. Values above `4` are clamped, and the caret is always kept inside the pane body. |
